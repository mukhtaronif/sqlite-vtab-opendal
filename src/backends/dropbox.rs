...
//!
//! This backend allows querying Dropbox files and folders using SQL.
//! Requires a Dropbox access token for authentication.
...
///
/// This backend uses OpenDAL's Dropbox service to list files from Dropbox.
///
/// # Authentication
///
/// Requires a Dropbox access token. You can obtain one from:
...
///
/// let backend = DropboxBackend::new("YOUR_ACCESS_TOKEN", "/");
...
pub struct DropboxBackend {
    ...
    access_token: String,
    /// Base path in Dropbox (e.g., "/" for root)
    base_path: String,
}

impl DropboxBackend {
    /// Create a new Dropbox backend
    ///
    /// # Arguments
    ///
    ...
    /// use sqlite_vtable_opendal::backends::dropbox::DropboxBackend;
    ///
    /// let backend = DropboxBackend::new("token", "/Documents");
    /// ```
    ...
    /// Create an OpenDAL operator for Dropbox
...
/// Register the dropbox virtual table with SQLite
///
/// This function creates a virtual table module that allows querying
/// Dropbox files using SQL.
///
/// # Arguments
///
/// * `conn` - SQLite connection
/// * `module_name` - Name for the virtual table (e.g., "dropbox_files")
/// * `access_token` - Dropbox access token
/// * `base_path` - Base path in Dropbox (e.g., "/" for root)
///
/// # Example
///
/// ```rust,ignore
...
/// dropbox::register(&conn, "dropbox_files", "YOUR_TOKEN", "/")?;
///
/// // Now you can query: SELECT * FROM dropbox_files
/// ```
pub fn register(
    ...
    // Create a specific table type for Dropbox
    #[repr(C)]
    struct DropboxTable {
    ...
    // Create a specific cursor type for Dropbox
    #[repr(C)]
    ...
    unsafe impl VTabCursor for DropboxCursor {
        fn filter(
            &mut self,
            _idx_num: c_int,
            _idx_str: Option<&str>,
    ...
    impl vtab::CreateVTab<'_> for DropboxTable {
        const KIND: VTabKind = VTabKind::EponymousOnly;
    }

    unsafe impl VTab<'_> for DropboxTable {
        type Aux = (String, String);
        type Cursor = DropboxCursor;
    ...
    conn.create_module(
        module_name,
        eponymous_only_module::<DropboxTable>(),
        Some((token, path)),
    )
}
...
mod tests {
    ...
    fn test_backend_creation() {
        let backend = DropboxBackend::new("test_token", "/Documents");
        assert_eq!(backend.access_token, "test_token");
        assert_eq!(backend.base_path, "/Documents");
        assert_eq!(backend.backend_name(), "dropbox");
    }

    #[test]
    fn test_backend_with_root_path() {
        let backend = DropboxBackend::new("token", "/");
        assert_eq!(backend.base_path, "/");
    }

    // Note: Integration tests with actual Dropbox API would require credentials
